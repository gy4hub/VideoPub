"""元数据解析（JSON / Markdown / TXT / Word / PDF → PublishTask）"""
...
def _find_metadata_file(folder_path: Path) -> Path:
    """在文件夹中查找元数据文件，优先级: .json > .md > .txt > .docx > .pdf"""
    for ext in (".json", ".md", ".txt", ".docx", ".pdf"):
    ...
    raise FileNotFoundError(
        f"在 {folder_path} 中未找到元数据文件 (metadata.json/.md/.txt/.docx/.pdf)"
    )
...
def _resolve_asset_path(folder_path: Path, raw_path: str | None, fallback_finder) -> Path:
...
def _cast_field_value(field: str, raw: str):
    ...
    raw = raw.strip()
    if field == "tags":
        hashtag_tags = [tag.strip() for tag in re.findall(r"#([^\s#，,、]+)", raw) if tag.strip()]
        if hashtag_tags:
            return hashtag_tags
        return [tag.strip().lstrip("#") for tag in re.split(r"[,，、]", raw) if tag.strip()]
...
def _parse_json(file_path: Path, folder_path: Path) -> PublishTask:
    ...
    return PublishTask(video_path=video_path, cover_path=cover_path, platforms=platforms)


def _parse_kv_text(text: str, folder_path: Path) -> PublishTask:
    """解析 key: value 纯文本，支持 [platform] 节头。"""
    global_data: dict = {}
    platforms: list[PlatformMeta] = []
    ...
    def _flush_platform():
        ...
        if current_platform_raw and current_fields:
            try:
                platform = _parse_platform(current_platform_raw)
                collection = current_fields.get("collection")
                if collection is None and platform == Platform.WECHAT:
    ...
    for line in text.splitlines():
        ...
        if header_match:
            _flush_platform()
            current_platform_raw = header_match.group(1).strip()
            current_fields = {}
            continue
...
def _parse_text_file(file_path: Path, folder_path: Path) -> PublishTask:
    """解析 Markdown / TXT 元数据文件。"""
    text = file_path.read_text(encoding="utf-8")
    return _parse_kv_text(text, folder_path)
...
def parse(folder_path: Path) -> PublishTask:
    ...
    支持格式: JSON (.json) / Markdown (.md) / TXT (.txt) / Word (.docx) / PDF (.pdf)
    """
    ...
    if meta_file.suffix in (".md", ".txt"):
        return _parse_text_file(meta_file, folder_path)
